...
const OrderDetails = () => {
    ...
    // Math Logic
    const totalMRP = order?.items?.reduce((total, item) => total + ((item.original_price || item.price) * item.quantity), 0) || 0;
    const totalSellingPrice = order?.items?.reduce((total, item) => total + (item.price * item.quantity), 0) || 0;
