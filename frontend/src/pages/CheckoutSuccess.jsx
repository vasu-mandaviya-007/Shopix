...
const CheckoutSuccess = () => {
    ...
    if (loading) {
        return (
            <div className='flex flex-col items-center justify-center min-h-[90vh] text-center px-4'>
                <div className="w-12 h-12 border-4 border-gray-200 border-t-green-500 rounded-full animate-spin mb-5" />
                <h2 className='text-lg font-semibold text-gray-700 mb-1'>Verifying Payment...</h2>
                <p className='text-sm text-gray-400'>Please do not close or refresh this window.</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className='min-h-[80vh] flex flex-col items-center justify-center gap-4 text-center px-4 py-16'>

                <div className="relative w-20 h-20 mx-auto mb-4">
                    <div className="absolute inset-0 rounded-full bg-red-100 animate-ping opacity-60" />
                    <div className="absolute inset-2 rounded-full bg-red-50 animate-ping opacity-40" style={{ animationDelay: '0.5s' }} />
                    <div className="relative z-10 w-full h-full rounded-full bg-red-500 flex items-center justify-center shadow-lg">
                        <svg className="w-8 h-8" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M16 16 L36 36 M36 16 L16 36" stroke="white" strokeWidth="4" strokeLinecap="round" />
                        </svg>
                    </div>
                </div>

                <h2 className='text-red-500 text-xl sm:text-2xl font-bold'>Oops! Something went wrong.</h2>
                <p className='text-gray-500 text-sm sm:text-base max-w-sm leading-relaxed'>{error}</p>
                <Button
                    variant="contained"
                    onClick={() => navigate('/profile/orders')}
                    className='rounded-lg! mt-2 capitalize!'
                >
                    Go to My Orders
                </Button>
            </div>
    ...
    return (
        ...
        <div className="min-h-screen bg-gray-50 py-6 sm:py-10 px-3 sm:px-4 font-sans text-gray-800">
            ...
            <style>{`
                ...
                .ripple-ring { position: absolute; inset: 0; border-radius: 50%; background: rgba(34,197,94,0.2); animation: ripplePulse 2s infinite; }
                .ripple-ring-2 { animation-delay: 1s; }
            `}</style>

            <div className="max-w-3xl mx-auto">

                {/* ── Success Header ── */}
                <div className="bg-white rounded-lg sm:rounded-2xl px-6 py-12 sm:px-12 sm:py-20 mb-4 sm:mb-6 text-center shadow-sm border border-gray-100">
                    ...
                    <div className="relative w-20 h-20 mx-auto mb-6">
                        ...
                        <svg
                            className="checkmark relative z-10"
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 52 52"
                            style={{
                                width: '100%', height: '100%', borderRadius: '50%',
                                display: 'block', strokeWidth: '4', stroke: '#fff',
                                strokeMiterlimit: '10',
                                boxShadow: 'inset 0px 0px 0px #22c55e',
                            }}
                        >
                            <circle
                                className="checkmark__circle"
                                cx="26" cy="26" r="25" fill="none"
                                style={{ strokeWidth: '2', strokeMiterlimit: '10', stroke: '#22c55e' }}
                            />
                            <path
                    ...
                    <span className="inline-block bg-green-50 text-green-600 text-xs font-bold px-3 py-1 rounded-full mb-3 tracking-wide uppercase">
                        Payment Confirmed
                    </span>
                    ...
                    <p className="text-sm sm:text-base text-gray-500 leading-relaxed">
                        Hey <span className="font-semibold text-gray-700">{orderData?.full_name}</span>, your order has been confirmed
                        and will be shipped within 2 business days.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-6">
                    ...
                    <div className="bg-white group transition-all duration-200 rounded-md sm:rounded-lg p-3 sm:p-4 border border-gray-100 shadow-sm flex flex-col gap-1.5">
                        <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 group-hover:bg-blue-500 rounded-sm bg-blue-50 flex items-center justify-center">
                            <MdOutlineLocalShipping className="text-blue-500 group-hover:text-white text-sm sm:text-base" />
                        </div>
                        <p className="text-mobile-1 sm:text-xs text-gray-400 font-medium">Est. Delivery</p>
                        <p className="text-xs sm:text-sm font-bold text-gray-800 leading-tight">{orderData?.created_at}</p>
                    </div>

                    <div className="bg-white group transition-all duration-200 rounded-md sm:rounded-lg p-3 sm:p-4 border border-gray-100 shadow-sm flex flex-col gap-1.5">
                        <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 group-hover:bg-purple-500 rounded-sm bg-purple-50 flex items-center justify-center">
                            <BsCreditCard2Back className="text-purple-500 group-hover:text-white text-sm sm:text-base" />
                        </div>
                        <p className="text-mobile-1 sm:text-xs text-gray-400 font-medium">Payment</p>
                        <p className="text-xs sm:text-sm font-bold text-gray-800 leading-tight">{orderData?.payment_method}</p>
                    </div>

                    <div className="col-span-2 sm:col-span-1 bg-white group transition-all duration-200  rounded-md sm:rounded-lg p-3 sm:p-4 border border-gray-100 shadow-sm flex flex-col gap-1.5">
                        <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 group-hover:bg-orange-500 rounded-sm bg-orange-50 flex items-center justify-center">
                            <FiHash className="text-orange-500 group-hover:text-white text-sm sm:text-base" />
                        </div>
                        <p className="text-mobile-1 sm:text-xs text-gray-400 font-medium">Order ID</p>
                        <p className="text-mobile-1 sm:text-xs font-bold text-indigo-500 leading-tight truncate">{orderData?.uid}</p>
                    </div>
                ...
                {/* ── Track Order Button ── */}
                <button
                    onClick={() => navigate(`/order_details/${orderData?.uid}`)}
                    className="w-full cursor-pointer bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98] text-white text-sm font-semibold px-5 py-3 sm:py-3.5 rounded-lg transition-all mb-4 sm:mb-6 flex items-center justify-center gap-2"
                >
                ...
                {/* ── Order Items ── */}
                <div className="bg-white rounded-md sm:rounded-lg border border-gray-100 shadow-sm overflow-hidden mb-4 sm:mb-6">
                    ...
                    <div className="divide-y divide-gray-50">
                        {orderData?.items?.map((item, index) => (
                            <div key={index} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="text-xs sm:text-sm font-semibold text-gray-800 line-clamp-2 leading-snug mb-1">
                                        {item?.product_name}
                                        {item?.attribute_values?.length > 0 && (
                                            <span className="text-gray-400 font-normal">
                                                {" "}({item.attribute_values.map(a => a.value).join(" / ")})
                                            </span>
                                        )}
                                    </h4>
                                    <span className="inline-block bg-gray-100 text-gray-500 text-mobile-1 sm:text-xs font-medium px-2 py-0.5 rounded-md">
                                        Qty: {item?.quantity}
                                    </span>
                                </div>
                                ...
                                <div className="shrink-0 text-right">
                                    ...
                                    {Number(item?.original_price) > Number(item?.price) && (
                                        <p className="text-mobile-1 sm:text-xs text-gray-400 line-through">
                                            {formatPriceINR(item?.original_price)}
                                        </p>
                                    )}
                                </div>
                ...
                <div className="bg-white rounded-md sm:rounded-lg border border-gray-100 shadow-sm overflow-hidden mb-4 sm:mb-6">
                    ...
                    <div className="p-4 sm:p-6 space-y-3">
                        ...
                        <div className="flex justify-between text-sm text-gray-600">
                            <span>Subtotal (MRP)</span>
                            <span className="font-medium text-gray-800">{formatPriceINR(totalMRP)}</span>
                        </div>
                        ...
                        {orderData?.coupon_used && (
                            <div className="flex justify-between text-sm text-gray-600">
                                <span>Coupon
                                    <span className="ml-1.5 bg-green-50 text-green-600 text-mobile-1 sm:text-xs font-bold px-1.5 py-0.5 rounded-md">
                                        {orderData?.coupon_used}
                                    </span>
                                </span>
                                <span className="text-green-600 font-medium">−{formatPriceINR(orderData?.discount_amount)}</span>
                        ...
                        <div className="flex justify-between text-gray-900 font-black text-base sm:text-lg pt-1">
                            <span>Total Paid</span>
                            <span className="text-indigo-600">{formatPriceINR(orderData?.total_amount)}</span>
                        </div>
                ...
                <div className="text-center py-4 sm:py-6">
                    <p className="text-xs sm:text-sm text-gray-400 mb-4">
                        Thank you for shopping with <span className="font-bold text-gray-600">Shopix</span> 🎉
                    </p>
                    <Button
                        variant="dark"
                        size="large"
                        onClick={() => navigate("/")}
                        className='rounded-xl! max-sm:px-4!'
                    >
                        Continue Shopping
                    </Button>
                    {/* <Button
                        ...
                        size="large"
                        onClick={() => navigate("/")}
                        className='rounded-xl! capitalize! shadow-none! bg-gray-900! hover:bg-gray-800! px-8! text-sm! sm:text-base!'
                    >
                        Continue Shopping
                    </Button> */}
                </div>
