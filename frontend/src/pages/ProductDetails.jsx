...
const ProductDetails = () => {
    ...
    // 🌟 2. useQuery Hook (Replaces useEffect + useState for fetching)
    const {
        data: product,
        isLoading: loading,
        isError,
        ...
        queryKey: ['product', slug], // Slug change hoga toh apne aap fetch hoga
        queryFn: fetchProductData,
        retry: 1, // Agar API fail ho jaye toh 1 baar aur try karega
    });

    // 🌟 3. Derived State with useMemo (Optimized)
    // Ye logic sirf tab chalega jab 'product' data actual me change hoga
    const { attributesMap, allKeys } = useMemo(() => {
        if (!product || !product.variants) return { attributesMap: {}, allKeys: [] };
        ...
        const keys = product.variants[0]?.attribute_values.map((attr) => attr.attribute_name) || [];

        return { attributesMap: map, allKeys: keys };
    }, [product]);
    ...
    const handleSelect = (key, value) => {
        ...
        if (bestMatch) {
            ...
            if (fallback) setSearchParams({ variant: fallback.uid });
        }
    };
    ...
    if (isError || !product) {
        return (
            <div className='flex items-center justify-center h-64 sm:h-96 text-sm sm:text-base text-gray-500'>
                Product Not Found
            </div>
        );
    }
    ...
    return (
        ...
        <div className='container px-5 sm:px-4 lg:px-10 mx-auto mt-4 sm:mt-6 lg:mt-8'>

            {/* Share Backdrop */}
            <div
                className={`fixed w-full h-full top-0 left-0 bg-black/30 z-50 ${showSharePopup ? "visible" : "invisible"}`}
                onClick={() => setshowSharePopup(false)}
            />

            {/* Share Popup — responsive width */}
            <div className={`
                        transition-all z-50
                        ${showSharePopup ? "-translate-y-1/2 opacity-100 pointer-events-auto duration-300" : "-translate-y-2/3 opacity-0 pointer-events-none duration-0"}
                        fixed top-1/2 left-1/2 -translate-x-1/2
                        w-[90vw] sm:w-120
                        rounded-xl bg-white shadow-xl border border-gray-300
                    `}>
                <div className='relative p-4 sm:p-6'>
                    <div className='mb-2 flex items-center justify-between'>
                        <h4 className='font-semibold text-base'>Share</h4>
                        <IoClose
                            onClick={() => setshowSharePopup(false)}
                            className='w-8 h-8 p-1.5 text-black/70 hover:text-red-500 rounded-full cursor-pointer hover:bg-gray-100'
                        />
                    </div>
                    <div className='mb-2'>
                        <div style={{ scrollbarWidth: "thin" }} className='flex items-center overflow-x-auto mt-4 gap-1 sm:gap-2'>
                            <a href={`https://wa.me/?text=${window.location.href}`} target='_blank' className='hover:bg-gray-100 p-1.5 sm:p-2 rounded-sm text-center shrink-0'>
                                <svg className='w-10 sm:w-13 rounded-full' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#000000"><rect width="512" height="512" rx="15%" fill="#25d366"></rect><path fill="#25d366" stroke="#ffffff" strokeWidth="26" d="M123 393l14-65a138 138 0 1150 47z"></path><path fill="#ffffff" d="M308 273c-3-2-6-3-9 1l-12 16c-3 2-5 3-9 1-15-8-36-17-54-47-1-4 1-6 3-8l9-14c2-2 1-4 0-6l-12-29c-3-8-6-7-9-7h-8c-2 0-6 1-10 5-22 22-13 53 3 73 3 4 23 40 66 59 32 14 39 12 48 10 11-1 22-10 27-19 1-3 6-16 2-18"></path></svg>
                                <span className='text-mobile-1 sm:text-xs'>WhatsApp</span>
                            </a>
                            <a href={`https://www.facebook.com/sharer/sharer.php?u=${window.location.href}`} target='_blank' className='hover:bg-gray-100 p-1.5 sm:p-2 rounded-sm text-center shrink-0'>
                                <svg className='w-10 sm:w-13 rounded-full' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#000000"><rect width="512" height="512" rx="15%" fill="#1877f2"></rect><path d="M355.6 330l11.4-74h-71v-48c0-20.2 9.9-40 41.7-40H370v-63s-29.3-5-57.3-5c-58.5 0-96.7 35.4-96.7 99.6V256h-65v74h65v182h80V330h59.6z" fill="#ffffff"></path></svg>
                                <span className='text-mobile-1 sm:text-xs'>Facebook</span>
                            </a>
                            <a href={`https://t.me/share/url?url=${window.location.href}`} target='_blank' className='hover:bg-gray-100 p-1.5 sm:p-2 rounded-sm text-center shrink-0'>
                                <svg className='w-10 sm:w-13 rounded-full' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#000000"><rect width="512" height="512" rx="15%" fill="#37aee2"></rect><path fill="#c8daea" d="M199 404c-11 0-10-4-13-14l-32-105 245-144"></path><path fill="#a9c9dd" d="M199 404c7 0 11-4 16-8l45-43-56-34"></path><path fill="#f6fbfe" d="M204 319l135 99c14 9 26 4 30-14l55-258c5-22-9-32-24-25L79 245c-21 8-21 21-4 26l83 26 190-121c9-5 17-3 11 4"></path></svg>
                                <span className='text-mobile-1 sm:text-xs'>Telegram</span>
                            </a>
                            <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${window.location.href}`} target='_blank' className='hover:bg-gray-100 p-1.5 sm:p-2 rounded-sm text-center shrink-0'>
                                <svg className='w-10 sm:w-13 rounded-full' viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="none"><path fill="#0A66C2" d="M12.225 12.225h-1.778V9.44c0-.664-.012-1.519-.925-1.519-.926 0-1.068.724-1.068 1.47v2.834H6.676V6.498h1.707v.783h.024c.348-.594.996-.95 1.684-.925 1.802 0 2.135 1.185 2.135 2.728l-.001 3.14zM4.67 5.715a1.037 1.037 0 01-1.032-1.031c0-.566.466-1.032 1.032-1.032.566 0 1.031.466 1.032 1.032 0 .566-.466 1.032-1.032 1.032zm.889 6.51h-1.78V6.498h1.78v5.727zM13.11 2H2.885A.88.88 0 002 2.866v10.268a.88.88 0 00.885.866h10.226a.882.882 0 00.889-.866V2.865a.88.88 0 00-.889-.864z"></path></svg>
                                <span className='text-mobile-1 sm:text-xs'>LinkedIn</span>
                            </a>
                            <a href={`https://twitter.com/intent/tweet?url=${window.location.href}`} target='_blank' className='hover:bg-gray-100 p-1.5 sm:p-2 rounded-sm text-center shrink-0'>
                                <svg className='w-10 sm:w-13 rounded-full' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="#000000"><rect width="512" height="512" rx="15%" fill="#1da1f2"></rect><path fill="#ffffff" d="M437 152a72 72 0 01-40 12a72 72 0 0032-40a72 72 0 01-45 17a72 72 0 00-122 65a200 200 0 01-145-74a72 72 0 0022 94a72 72 0 01-32-7a72 72 0 0056 69a72 72 0 01-32 1a72 72 0 0067 50a200 200 0 01-105 29a200 200 0 00309-179a200 200 0 0035-37"></path></svg>
                                <span className='text-mobile-1 sm:text-xs'>Twitter</span>
                            </a>
                        </div>

                        <div className='p-2 mt-4 pl-3 sm:pl-4 border border-gray-300 rounded-lg flex items-center gap-2'>
                            <input
                                type="text"
                                value={window.location.href}
                                className='w-full border-none pb-0.5 text-gray-600 text-xs sm:text-sm outline-none truncate'
                                readOnly
                            />
                            <button
                                onClick={handleCopyUrl}
                                className='bg-blue-500 px-3 sm:px-4 py-1.5 shrink-0 disabled:bg-gray-500 disabled:cursor-not-allowed font-semibold rounded-full text-xs sm:text-sm text-white hover:bg-blue-600 cursor-pointer'
                            >
                                Copy
                            </button>
                        </div>
            ...
            {/* Breadcrumb */}
            <div className='hidden sm:flex items-center gap-2 text-xs mb-4 sm:mb-6 text-nowrap overflow-clip'>
                <Link to={"/"} className='text-[#878787] cursor-pointer hover:text-blue-400 font-medium'>Home</Link>
                {product?.category_path.map(cat => (
                    <div className='flex items-center' key={cat.uid}>
                        <span className='text-gray-500 mr-2'><FaAngleRight /></span>
            ...
            <div className='flex items-start flex-col md:flex-row justify-between xl:gap-20 lg:gap-10 gap-4 sm:gap-6'>
                ...
                {/* Left — Images */}
                <div className='w-full md:min-w-5/11 md:max-w-5/11 md:sticky md:z-10 md:top-25'>
                    ...
                    <div className='relative flex gap-3 sm:gap-5 lg:gap-7 w-full mb-6 sm:mb-10'>
                        ...
                        <div className='w-full sm:w-9/10 sm:min-w-9/10'>
                            ...
                            {/* Thumbnail Strip */}
                            <div style={{ scrollbarWidth: "thin" }} className='w-full max-w-full min-w-full mt-3 sm:mt-4 overflow-x-auto'>
                                <div className='flex gap-2 sm:gap-3'>
                                    {selectedVariant?.images?.slice(0, 10).map((img, index) => (
                                        <div
                                            key={index}
                                            onMouseOver={() => handleImageHover(img?.image)}
                                            className={`cursor-pointer ${activeImage === img?.image ? "border-2 border-blue-600" : "border border-gray-300"} min-w-14 h-14 sm:min-w-18 sm:h-18 md:min-w-20 md:h-20! p-1 rounded-xs`}
                                        >
                                            <div className='relative p-1 h-full overflow-hidden w-full'>
                                                <img src={img?.image} className='absolute top-1/2 left-1/2 -translate-1/2 max-h-full! max-w-full!' alt="" />
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className='max-sm:absolute max-sm:right-1 max-sm:top-2 max-sm:z-10 w-1/10 flex flex-col gap-2 sm:gap-4 '>
                            <button
                                onClick={() => setshowSharePopup(true)}
                                className='w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-sm bg-[#efefef] text-gray-700 hover:bg-gray-200 cursor-pointer text-sm sm:text-base'
                            >
                                <RxShare2 />
                            </button>

                            {/* NAYA CODE (Working Wishlist Button) */}
                            {
                                selectedVariant?.uid && (
                                    <div className='w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-sm text-gray-700 cursor-pointer text-sm sm:text-base'>
                                        <WishlistButton productId={selectedVariant?.uid} />
                                    </div>
                                )
                            }

                            {/* <button className='w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-sm bg-[#efefef] text-gray-700 hover:bg-gray-200 cursor-pointer text-sm sm:text-base'>
                                <BsHeart />
                            </button> */}
                ...
                <div className='w-full md:w-6/11 md:max-w-6/11'>
                    ...
                    <p className='font-semibold text-blue-500 text-sm sm:text-base'>{product?.product?.brand}</p>

                    <h1 className='font-bold text-sm sm:text-base xl:text-lg mt-2 sm:mt-4 leading-snug'>
                        {product?.product?.title}
                        {" "}
                        ({selectedVariant && selectedVariant.attribute_values.map(item => item.value).join(" / ")})
                    </h1>

                    {/* Price & Rating */}
                    <div className='mt-4 sm:mt-6 flex items-start sm:items-center justify-between gap-3 flex-wrap'>
                        <div className='flex items-end gap-2 sm:gap-4 flex-wrap'>
                            {selectedVariant?.price > 0 ? (
                                <>
                                    <span className='font-bold text-xl sm:text-2xl'>{formatPriceINR(selectedVariant?.price)}</span>
                                    {selectedVariant?.mrp > 0 && <strike className='text-gray-400 text-sm sm:text-base'>{formatPriceINR(selectedVariant?.mrp)}</strike>}
                                </>
                            ) : (
                                <span className='font-bold text-xl sm:text-2xl'>{formatPriceINR(selectedVariant?.mrp)}</span>
                            )}
                            <h4 className='inline-flex items-center gap-2 sm:gap-3'>
                                {selectedVariant?.discount_percent > 0 && (
                                    <p className='text-green-600 font-semibold text-sm'>{selectedVariant?.discount_percent}% off</p>
                                )}
                                {selectedVariant?.discount_percent > 0 && (
                                    <div className='relative group'>
                                        <CiCircleInfo className='text-base sm:text-lg text-gray-400' />
                                        <div className='absolute text-xs text-nowrap bg-white z-10 invisible opacity-0 scale-y-0 delay-0 duration-300 transition-opacity origin-top group-hover:visible group-hover:opacity-100 group-hover:scale-y-100 group-hover:delay-1000 border border-gray-200 shadow-xl left-0 top-full'>
                                            <div>
                                                <h3 className='font-semibold px-4 pt-4 text-sm'>Price Details</h3>
                                                <div className='px-4'>
                                                    <div className='flex my-4 justify-between gap-6'>
                                                        <div>
                                                            <p className='text-nowrap font-semibold text-gray-500 text-xs sm:text-sm'>Maximum Retail Price</p>
                                                            <p className='text-xs text-gray-400'>(incl. of all taxes)</p>
                                                        </div>
                                                        <span className='line-through text-sm'>₹{product?.product?.display_variant?.price}</span>
                                                    </div>
                                                    <div className='flex justify-between pb-3 border-dashed border-b border-b-gray-300 gap-6'>
                                                        <p className='text-nowrap font-semibold text-gray-500 text-xs sm:text-sm'>Selling Price</p>
                                                        <span className='line-through text-sm'>₹{product?.product?.display_variant?.discount_price}</span>
                                                    </div>
                                                    <div className='flex justify-between py-3 border-solid border-b border-b-gray-300 gap-6'>
                                                        <p className='text-nowrap font-semibold text-xs sm:text-sm'>Special Price</p>
                                                        <span className='text-sm'>₹{product?.product?.display_variant?.discount_price}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className='px-4 py-3'>
                                                <p className='text-green-500 text-xs sm:text-sm'>
                                                    Overall you save ₹{product?.product?.display_variant?.price - product?.product?.display_variant?.discount_price} ({product?.product?.display_variant?.discount_percent}%) on this product
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="flex items-center gap-1">
                            <Rating value={Number(product.product.average_rating)} precision={0.5} readOnly size="small" />
                            <p className='text-xs text-gray-400 font-semibold'>(<span>{product.product.total_reviews} </span> Reviews)</p>
                        </div>
                    ...
                    <div className='py-4 sm:py-5 my-4 sm:my-5 border-t border-b border-gray-200'>
                        ...
                        <h1 className='font-bold text-sm sm:text-base mb-3 sm:mb-4'>
                        ...
                        <div className='grid gap-4 sm:gap-6'>
                            {allKeys && allKeys.map((key) => {
                                ...
                                return (
                                    <div key={key}>
                                        <h3 className='text-black/60 font-semibold text-xs sm:text-sm'>
                                            {key.charAt(0).toUpperCase() + key.slice(1).toLowerCase()} : {selectedVariant?.attribute_values.find((a) => a.attribute_name === key)?.value}
                                        </h3>
                                        <div className='flex items-center flex-wrap gap-2 sm:gap-3 md:gap-4 mt-3 sm:mt-4'>
                                            {getUniqueValuesForKey(key).map(val => {
                                                const isActive = selectedVariant?.attribute_values.find(
                                                    (a) => a.attribute_name === key && a.value === val
                                                );
                                                ...
                                                if (isColorAttribute) {
                                                    return (
                                                        <button
                                                            ...
                                                            onClick={() => handleSelect(key, val)}
                                                            className={`
                                                                        w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 cursor-pointer rounded-md border-2 overflow-hidden relative transition-all duration-200
                                                                        ${isActive ? "border-blue-600 ring-2 ring-blue-100 ring-offset-1" : "border-gray-200 hover:border-gray-400"}
                                                                        ${!isAvailable && !isActive ? "opacity-40 grayscale cursor-not-allowed" : ""}
                                                                    `}
                                                            title={val}
                                                        >
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        onClick={() => handleSelect(key, val)}
                                                        className={`
                                                                    border-2 px-2 py-1.5 sm:p-2 min-w-[20%] sm:min-w-1/5 cursor-pointer rounded-md bg-gray-50 text-xs text-gray-700 font-medium
                                                                    ${isActive ? "border-sky-600 bg-blue-50! font-semibold" : "border-gray-200"}
                                                                    ${!isAvailable && !isActive ? "border-dashed border-gray-300 text-gray-400 bg-gray-50" : "border-gray-200"}
                                                                `}
                                                    >
                                                        {val}
                                                    </button>
                    ...
                    {/* CTA Buttons */}
                    <div className='mt-6 sm:mt-10 flex w-full gap-2 sm:gap-4'>
                        <Button
                            startIcon={<FaCartPlus />}
                            loading={cartLoading}
                            loadingPosition='start'
                            loadingIndicator={<PiSpinner className='text-base sm:text-lg animate-spin' />}
            ...
            <div className="mt-10 sm:mt-16">
                ...
                <div className="border-b border-gray-200 overflow-x-auto overflow-y-hidden">
                    <nav className="-mb-px flex space-x-4 sm:space-x-8 min-w-max">
                        <button
                            ...
                            className={`tab-button ${activeTab === 'description' ? 'active' : ''} border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm`}
                        >
                            Description
                        </button>
                        <button
                            onClick={() => showTab('specifications')}
                            className={`tab-button ${activeTab === 'specifications' ? 'active' : ''} border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm`}
                        >
                            Specifications
                        </button>
                    </nav>
                ...
                <div className="py-4 sm:py-8">
                    ...
                    {activeTab === "description" && (
                        <div className="tab-content border border-gray-200 rounded-md p-4 sm:p-6 lg:p-10">
                            <div className="prose max-w-none">
                                <h3 className="text-lg sm:text-2xl font-bold mb-4 sm:mb-8 pb-3 sm:pb-4 border-b border-gray-300">
                                    Product Description
                                </h3>
                                <div
                    ...
                    {activeTab === "specifications" && (
                        <div className="tab-content border border-gray-200">
                            <div className="prose max-w-none">
                                <h3 className="text-lg sm:text-2xl font-bold px-4 sm:px-5 py-3 sm:py-4 border-b border-b-gray-200">
                                    Specifications
                                </h3>
                                {product?.product?.specifications?.map((spec, index) => (
                                    <div key={index} className='p-4 sm:p-6 border-b border-b-gray-200'>
                                        <h4 className='font-semibold mb-4 sm:mb-6 text-sm sm:text-base'>{spec?.name}</h4>
                                        <table className='w-full relative'>
                                            <span className='hidden sm:block h-full w-px bg-gray-200 left-1/2 -translate-x-1/2 absolute' />
                                            <tbody className='grid grid-cols-2 md:grid-cols-2 gap-x-8 gap-y-4 sm:gap-10'>
                                                {spec?.spec_items?.map((item) => (
                                                    <tr key={item?.id} className='flex flex-col sm:flex-row gap-1 sm:gap-0 border-b border-b-gray-200 sm:border-none pb-4 sm:pb-0'>
                                                        <td className='w-full sm:w-1/2 text-xs sm:text-sm text-[#878787]'>{item?.name}</td>
                                                        <td className='w-full sm:w-1/2 text-xs sm:text-[13px] font-normal text-[#323232]'>{item?.value}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
            ...
            <ProductReviews productId={product.product.uid} onReviewAdded={refetch} />
