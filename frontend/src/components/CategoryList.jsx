...
const CategoryList = ({ className = "" }) => {
    ...
    return (
        ...
        <div className={`${className} mt-3 sm:mt-4`}>
            ...
            <div className='w-full grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2 sm:gap-3 md:gap-5 lg:gap-8 mt-4 sm:mt-6 lg:mt-8'>
                ...
                {loading
                    ...
                    : categories.map((category, index) => (
                        ...
                        <div
                            key={index}
                            onClick={() => { navigate(`/products/${category.slug}`); scrollTo(0, 0); }}
                        ...
                        >
                            <div className='relative bg-gray-100 group-hover:brightness-95 transition-all rounded-full aspect-square w-full'>
                                <img
                                    ...
                                    className='p-1.5 sm:p-2 absolute left-1/2 top-1/2 -translate-1/2 max-h-full! max-w-full!'
                                    alt={category.name}
                                    loading='lazy'
                                />
                            </div>
                            <span className='font-semibold text-mobile-1 sm:text-xs md:text-[13px] lg:text-sm text-center mt-1.5 sm:mt-2 leading-tight'>
                                {category.name}
                            </span>
                        </div>
