...
const SpecListLayout = ({ product }) => {
    ...
    return (
        ...
        <div className="bg-white shadow-xs sm:shadow border border-gray-200 p-2 sm:p-3 lg:p-4 flex flex-col transition-shadow hover:shadow-md">

            <Link to={`/product/${product.product_slug}/?variant=${product.uid}`} className="text-sm font-medium text-gray-800 mb-2 leading-snug">

                {/* Image Area */}
                <div className="relative w-full overflow-hidden">

                    {isNewProduct(product.created_at) && (
                        <span className="absolute top-0 right-0 sm:top-2 sm:right-2 bg-gray-800 text-white px-1.5 py-0.5 z-2 text-9px sm:text-mobile-1 md:text-xs font-medium uppercase tracking-wide">
                            new
                        </span>
                    )}

                    <div className='h-36 sm:h-44 md:h-50 lg:h-55 xl:h-64 max-w-28 sm:max-w-32 lg:max-w-36 m-auto flex items-center justify-center bg-white'>
                        <img
                            src={product?.images?.filter(img => img.is_main)[0]?.image || product?.images[0]?.image}
                            alt={product.product_name}
                            className="absolute top-1/2 p-1 lg:p-3 left-1/2 -translate-1/2 max-w-full! max-h-full! m-auto"
                            loading='lazy'
                            onError={(e) => e.target.src = 'https://via.placeholder.com/500?text=Product+Image'}
                        />
                    </div>
                ...
                {/* Product Info */}
                <div className="flex mt-2 flex-col grow">

                    <div className="text-9px sm:text-mobile-1 lg:text-xs text-gray-400 uppercase tracking-wide mb-1 font-semibold">
                        {product?.primary_category}
                    </div>

                    <h1 className="text-mobile-2 sm:text-sm font-medium text-gray-800 mb-1 lg:mb-2 leading-snug line-clamp-2 sm:line-clamp-1">
                        {product.product_name}
                    </h1>

                    {/* Attributes — hidden on mobile */}
                    <div className='mb-1.5 max-md:hidden leading-snug line-clamp-1 text-mobile-2 lg:text-xs text-gray-500 font-medium'>
                        {product?.attribute_values?.slice(0, 2).map((attribute, index) => (
                            `${attribute?.attribute_name.charAt(0).toUpperCase() + attribute?.attribute_name.slice(1).toLowerCase()} : ${attribute?.value}${index !== product?.attribute_values?.slice(0, 2).length - 1 ? ', ' : ''}`
                        )).join("")}
                    </div>
                    ...
                    {/* Price Row */}
                    <div className='flex flex-col-reverse items-start gap-0.5 sm:gap-1 xl:flex-row xl:items-center xl:gap-3'>
                        <div className='flex items-center gap-2 sm:gap-3'>
                            {product?.mrp > 0 ? (
                                <>
                                    ...
                                    {product?.price > 0 && (
                                        <strike className='text-gray-400 text-xs lg:text-sm'>
                        ...
                        {product?.discount_percent > 0 && (
                            <p className='text-green-600 font-semibold text-mobile-1 sm:text-xs lg:text-sm'>
            ...
            {/* Add to Cart — visible on all screens, smaller on mobile */}
            {/* <div className="flex justify-end mt-auto pt-2 sm:pt-3 border-t border-gray-100">
                <Button
                    variant='contained'
                    size='small'
                    // className='bg-gray-800! w-full! lg:w-auto! text-xs! sm:text-sm!'
                    className='bg-gray-800! w-full! text-xs! whitespace-nowrap!'
                    onClick={() => addToCart(product)}
                    startIcon={<FaShoppingCart className='text-xs! sm:text-sm!' />}
                >
                    Add to Cart
                </Button>
            </div> */}
